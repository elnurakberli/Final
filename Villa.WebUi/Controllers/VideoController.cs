...
namespace Villa.WebUi.Controllers
{
    public class VideoController : Controller
    {
        ...
        public async Task<IActionResult> UptadeVideo(ObjectId id)
        {
            var value = await _videoService.TGetByIdAsync(id);
            var uptadeValue = _mapper.Map<UpdateVideoDto>(value);
            return View(uptadeValue);
        }

        [HttpPost]
        public async Task<IActionResult> UptadeVideo(UpdateVideoDto uptadeVideoDto)
        {
            var uptadeValue = _mapper.Map<Video>(uptadeVideoDto);
            await _videoService.TUptadeAsync(uptadeValue);
            return RedirectToAction("Index");
        }
    }
}
